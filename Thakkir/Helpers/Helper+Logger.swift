...
extension Helper {
    ...
    class Logger{
       ...
       class func write(string: String) {
            if let fcmToken = Messaging.messaging().fcmToken , let uuid  = UIDevice.current.identifierForVendor?.uuidString {
                
                
                CLSLogv("%@ ;Token:%@ ;uuid:%@", getVaList([string , fcmToken,uuid]))
       ...
       static private  let documentsDirectory: URL = {
